...
interface Phase1ComprehensiveProps {
  patient: Patient;
...
const Phase1ComprehensiveView: React.FC<Phase1ComprehensiveProps> = ({ 
    patient, 
    phase, 
    workflow, 
    setWorkflowData,
    ...
    const [openSections, setOpenSections] = useState<Record<string, boolean>>({ [testCategories[0]]: true });
    ...
    useEffect(() => {
        ...
        const testsTemplate = initialTests.reduce((acc, test) => {
        ...
        if (savedTests) {
            ...
            Object.keys(testsTemplate).forEach(key => {
                if (parsedTests[key]) {
                    testsTemplate[key] = { ...testsTemplate[key], ...parsedTests[key] };
                }
            });
            setTests(testsTemplate);
        } else {
            setTests(testsTemplate);
        }
    }, [storageKey, initialTests]);
    ...
    const { overallProgress, testsByCategory, categoryProgress, categoryAbnormalCounts, categoryExemptedCounts, totalAbnormal } = useMemo(() => {
        ...
        if (applicableTests.length === 0) {
            return { overallProgress: 0, testsByCategory: {}, categoryProgress: {}, categoryAbnormalCounts: {}, categoryExemptedCounts: {}, totalAbnormal: 0 };
        }
        ...
        const totalAbnormal = applicableTests.filter(t => t.isAbnormal).length;

        const testsByCategory = testCategories.reduce((acc, cat) => {
            acc[cat] = applicableTests.filter(t => t.category === cat);
            return acc;
        }, {} as Record<string, MedicalTestItem[]>);

        const categoryProgress = testCategories.reduce((acc, cat) => {
        ...
        const categoryAbnormalCounts = testCategories.reduce((acc, cat) => {
            const catTests = testsByCategory[cat];
            acc[cat] = catTests ? catTests.filter(t => t.isAbnormal).length : 0;
            return acc;
        }, {} as Record<string, number>);
        
        const categoryExemptedCounts = testCategories.reduce((acc, cat) => {
            const catTests = testsByCategory[cat];
            acc[cat] = catTests ? catTests.filter(t => t.isExempt).length : 0;
            return acc;
        }, {} as Record<string, number>);

        return { overallProgress, testsByCategory, categoryProgress, categoryAbnormalCounts, categoryExemptedCounts, totalAbnormal };
    }, [tests, patient, testCategories]);
    ...
    useEffect(() => {
        const handler = setTimeout(() => {
            ...
            if (phase.progress === overallProgress && phase.status === newStatus && phase.abnormalFindings === totalAbnormal) {
                return;
            }

            setWorkflowData(prev => prev.map(p => p.id === phase.id ? { ...p, progress: overallProgress, status: newStatus, abnormalFindings: totalAbnormal } : p));
        }, 500); // Debounce updates to parent to prevent focus loss

        return () => clearTimeout(handler);
    }, [overallProgress, phase.id, phase.progress, phase.status, phase.abnormalFindings, totalAbnormal, setWorkflowData]);
...
export default Phase1ComprehensiveView;
