...
interface Phase1RecipientProps {
  patient: Patient;
...
const Phase1RecipientComprehensive: React.FC<Phase1RecipientProps> = ({ patient, phase, workflow, setWorkflowData }) => {
    const [tests, setTests] = useState<Record<string, MedicalTestItem>>({});
    const [openSections, setOpenSections] = useState<Record<string, boolean>>({ [RECIPIENT_TEST_CATEGORIES[0]]: true });
    const [isLoadingSummary, setIsLoadingSummary] = useState(false);
    const [summary, setSummary] = useState('');
    
    // --- Data Persistence ---
    useEffect(() => {
        const storageKey = `phase1_recipient_tests_${patient.id}`;
        const savedTests = localStorage.getItem(storageKey);
        const initialTests = INITIAL_RECIPIENT_TESTS.reduce((acc, test) => {
    ...
    const { overallProgress, testsByCategory, categoryProgress, categoryAbnormalCounts, categoryExemptedCounts } = useMemo(() => {
        ...
        const testsByCategory = RECIPIENT_TEST_CATEGORIES.reduce((acc, cat) => {
            acc[cat] = applicableTests.filter(t => t.category === cat);
            return acc;
        }, {} as Record<string, MedicalTestItem[]>);

        const categoryProgress = RECIPIENT_TEST_CATEGORIES.reduce((acc, cat) => {
        ...
        const categoryAbnormalCounts = RECIPIENT_TEST_CATEGORIES.reduce((acc, cat) => {
            const catTests = testsByCategory[cat];
            acc[cat] = catTests ? catTests.filter(t => t.isAbnormal).length : 0;
            return acc;
        }, {} as Record<string, number>);
        
        const categoryExemptedCounts = RECIPIENT_TEST_CATEGORIES.reduce((acc, cat) => {
            const catTests = testsByCategory[cat];
    ...
    return (
        <div className="p-4 md:p-6 bg-white rounded-b-lg border border-t-0 border-gray-200 space-y-6">
            ...
            <div className="space-y-4">
                {RECIPIENT_TEST_CATEGORIES.map(category => {
...
export default Phase1RecipientComprehensive;
